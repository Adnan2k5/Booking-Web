...
export const restoreTermsVersion = async (title, version) => {
  try {
    const response = await axiosClient.post(`/api/terms/restore/${version}`, null, { params: { title } });
    return response.data.data;
  } catch (error) {
...
export const deleteTermsVersion = async (title, version) => {
  try {
    const response = await axiosClient.delete(`/api/terms/${version}`, { params: { title } });
    return response.data.data;
  } catch (error) {
