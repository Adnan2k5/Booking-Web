        ...
        {promos.map((p, i) => (
          <div key={i} className="relative h-[240px] rounded-xl overflow-hidden group">
            ...
            <div className="relative z-10 h-full flex flex-col justify-center p-8">
              ...
              <Button onClick={() => handleExplore(p.slug)} aria-label={`Explore ${p.slug}`} className="mt-4 bg-white text-black hover:bg-orange-500 hover:text-white rounded-full h-9 px-6 text-xs font-semibold tracking-wide w-fit">Explore</Button>
            </div>
          </div>
        ))}
      </div>
